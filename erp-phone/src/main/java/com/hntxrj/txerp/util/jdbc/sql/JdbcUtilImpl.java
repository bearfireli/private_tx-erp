...
import com.hntxrj.txerp.util.jdbc.BaseConnention;
...
public class JdbcUtilImpl implements JdbcUtil {
    ...
    public Object findOne(String sql, Param param, Class cs) throws JdbcUtilException {
        return findOne(sql, Collections.singletonList(param), cs);
    }
...
//        System.out.println(object);
    ...
    public DBTypeEnums getDBType() {
        String db = baseConnention.getDBType();
        db = db.toLowerCase();
        switch (db){
            case "mysql":
                return DBTypeEnums.MYSQL;
            case "sqlserver":
                return DBTypeEnums.SQLSERVER;
            default:
                return DBTypeEnums.MYSQL;
        }
    }
